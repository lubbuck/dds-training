...
Sistema de simulação de gestão de tarefas

> Status: Developed

## Funcionalidades

-   Listagem, Cadastro, Edição e Remoção de Tarefas
...
-   Após criar o arquivo `.env` edite as seguintes variáveis de ambiente
    ...
    DB_HOST=postgres
    DB_PORT=5432
    DB_DATABASE=dds_training_db
    DB_USERNAME=dds_training_user
    DB_PASSWORD=dds_training_password
    ```
...
-   Execute os próximos comandos em sequência
    ...
    -   Criar as tabelas no banco de dados com dados iniciais
        ...
        ```
    -   Rodar o Projeto em outro terminal
        ```bash
        composer run dev
        ```
